...
サイト(SSG + GitHub Pages)は `site/` に構築済み。構築計画は [`docs/site-plan.md`](docs/site-plan.md)、
実装TODOは [`docs/site-todo.md`](docs/site-todo.md) を参照。

## サイト(`site/`)

`data/json/` を入力に、日英対応のカード一覧・検索サイトを **SvelteKit + adapter-static** で
静的生成する。公開先は GitHub Pages(`https://rarejapanesenfts.github.io/directory/`)。

```bash
cd site
bun install
bun run dev        # 開発サーバ(http://localhost:5173)
bun run build      # 画像最適化 + 全 ~1,568 ページを build/ に静的生成
bun run preview    # build/ をローカル配信
...
- 本番の絶対URL(OGP/sitemap用)は環境変数 `PUBLIC_SITE_URL` で上書き可能。
